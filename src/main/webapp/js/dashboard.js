...
const DashboardModule = {
    ...
    calcularVentasSemanales: function(facturas) {
        ...
        this.state.diasSemana = data.map(d => d.dia);
    },
    ...
    // Actualizar gráfico de ventas semanales (Chart.js)
    actualizarGrafico: function() {
        const chart = Chart.getChart('ventasSemanaChart');
        if (!chart) return;

        chart.data.labels = this.state.diasSemana.slice();
        chart.data.datasets[0].data = this.state.ventasSemanales.slice();
        chart.update('none');
    },
